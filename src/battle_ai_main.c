...
static s32 AI_CheckBadMove(u32 battlerAtk, u32 battlerDef, u32 move, s32 score)
{
    ...
    if (!(moveTarget & MOVE_TARGET_USER))
    {
        ...
        if ((B_PRANKSTER_DARK_TYPES >= GEN_7 && IS_BATTLER_OF_TYPE(battlerDef, TYPE_DARK)
          && (aiData->abilities[battlerAtk] == ABILITY_PRANKSTER || BattlerHasPassiveAbility(battlerAtk, ABILITY_PRANKSTER)) && (IsBattleMoveStatus(move)))
...
u32 BattlerHasPassiveAbility(u8 battler, u16 passiveAbility)
{
    return SpeciesHasPassiveAbility(gBattleMons[battler].species, passiveAbility);
}
